...
fn retry_upstream_server_error_once(
    ...
    // 仅对白名单内的幂等 POST 接口执行 500 重试，避免重放带来重复生成/工具调用/计费。
    if method == reqwest::Method::POST {
        let is_idempotent = request_ctx.request_path.contains("/v1/embeddings")
            || request_ctx.request_path.contains("/v1/moderations");

        if !is_idempotent {
            if debug {
                log::warn!(
                    "event=gateway_skip_500_retry_non_idempotent path={} account_id={}",
...
mod tests {
    ...
    #[test]
    fn retries_server_error_once_for_safe_request_before_final_decision() {
    ...
    #[test]
    fn skips_server_error_retry_for_stateful_post_requests() {
        let storage = Storage::open_in_memory().expect("open storage");
        storage.init().expect("init storage");
        let now = now_ts();
        let account = build_account("acc-500-post-no-retry", now);
        let mut token = build_token(account.id.as_str(), now);
        let auth_token = token.access_token.clone();
        storage.insert_account(&account).expect("insert account");
        storage.insert_token(&token).expect("insert token");
        ...
        let join = thread::spawn(move || {
            ...
            request
                ...
                .expect("read request body");
            hit_count_thread.fetch_add(1, Ordering::SeqCst);
            let response = Response::from_string("first").with_status_code(StatusCode(500));
            request.respond(response).expect("respond");
        });
        ...
        match decision {
            PostRetryFlowDecision::RespondUpstream(resp) => assert_eq!(resp.status(), 500),
            _ => panic!("unexpected decision"),
        }
    }
}
