...
pub(super) fn try_handle(req: &JsonRpcRequest) -> Option<JsonRpcResponse> {
    let result = match req.method.as_str() {
        ...
        "account/update" => {
            ...
            let note = super::optional_string_param(req, "note");
            let tags = super::optional_string_param(req, "tags");
            super::ok_or_error(account_update::update_account(
                account_id,
                sort,
                status.as_deref(),
                label.as_deref(),
                note.as_ref().map(|value| value.as_deref()),
                tags.as_ref().map(|value| value.as_deref()),
            ))
        }
        "account/import" => {
